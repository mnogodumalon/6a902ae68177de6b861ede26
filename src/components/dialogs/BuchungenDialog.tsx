...
import type { Buchungen, Zimmer, LookupValue } from '@/types/app';
...
export function BuchungenDialog({ open, onClose, onSubmit, defaultValues, recordId, zimmerList, enablePhotoScan = true, enablePhotoLocation = true }: BuchungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['zimmer', 'anreisedatum', 'abreisedatum', 'vorname', 'nachname', 'email', 'anzahl_personen', 'status'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["zimmer"]);
        ...
        if (zimmerName) {
          const zimmerMatch = zimmerList.find(r => matchName(zimmerName!, [String(r.fields.zimmernummer ?? '')]));
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'zimmer': (
      <div key="zimmer" className="space-y-1.5">
        ...
        <Combobox
          id="zimmer"
          placeholder="Welches Zimmer buchen?"
          items={zimmerListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.zimmernummer ?? r.record_id),
          }))}
    ...
    'anreisedatum': (
      <div key="anreisedatum" className="space-y-1.5">
        <Label htmlFor="anreisedatum">{fieldLabel('buchungen', 'anreisedatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="anreisedatum"
        ...
        {showErrors && !fields.anreisedatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('buchungen', 'anreisedatum')}</p>
        )}
      </div>
    ),
    'abreisedatum': (
      <div key="abreisedatum" className="space-y-1.5">
        <Label htmlFor="abreisedatum">{fieldLabel('buchungen', 'abreisedatum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="abreisedatum"
        ...
        {showErrors && !fields.abreisedatum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('buchungen', 'abreisedatum')}</p>
        )}
      </div>
    ),
    'vorname': (
      <div key="vorname" className="space-y-1.5">
        <Label htmlFor="vorname">{fieldLabel('buchungen', 'vorname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="vorname"
          placeholder="z. B. Max"
        ...
        {showErrors && !fields.vorname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('buchungen', 'vorname')}</p>
        )}
      </div>
    ),
    'nachname': (
      <div key="nachname" className="space-y-1.5">
        <Label htmlFor="nachname">{fieldLabel('buchungen', 'nachname')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="nachname"
        ...
        {showErrors && !fields.nachname && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('buchungen', 'nachname')}</p>
        )}
      </div>
    ),
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('buchungen', 'email')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="email"
          type="email"
          inputMode="email"
        ...
        {showErrors && !fields.email && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('buchungen', 'email')}</p>
        )}
      </div>
    ),
    'telefon': (
      <div key="telefon" className="space-y-1.5">
        <Label htmlFor="telefon">{fieldLabel('buchungen', 'telefon')}</Label>
        <Input
          id="telefon"
          type="tel"
          inputMode="tel"
    ...
    'nachricht': (
      <div key="nachricht" className="space-y-1.5">
        <Label htmlFor="nachricht">{fieldLabel('buchungen', 'nachricht')}</Label>
        <Textarea
          id="nachricht"
          placeholder="Besondere Wünsche, Diäten, Fragen..."
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="status" className="max-sm:h-11"><SelectValue placeholder="z. B. Anfrage, Bestätigt" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="anfrage">{lookupLabel('buchungen', 'status', 'anfrage') ?? 'Anfrage'}</SelectItem>
            <SelectItem value="bestaetigt">{lookupLabel('buchungen', 'status', 'bestaetigt') ?? 'Bestätigt'}</SelectItem>
            <SelectItem value="eingecheckt">{lookupLabel('buchungen', 'status', 'eingecheckt') ?? 'Eingecheckt'}</SelectItem>
            <SelectItem value="ausgecheckt">{lookupLabel('buchungen', 'status', 'ausgecheckt') ?? 'Ausgecheckt'}</SelectItem>
            <SelectItem value="storniert">{lookupLabel('buchungen', 'status', 'storniert') ?? 'Storniert'}</SelectItem>
            <SelectItem value="abgelehnt">{lookupLabel('buchungen', 'status', 'abgelehnt') ?? 'Abgelehnt'}</SelectItem>
          </SelectContent>
        </Select>
  ...
  const FIELD_LABELS: Record<string, string> = {"zimmer": "Zimmer", "anreisedatum": "Anreisedatum", "abreisedatum": "Abreisedatum", "vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail", "telefon": "Telefon", "anzahl_personen": "Anzahl Personen", "nachricht": "Nachricht", "status": "Status"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"zimmer": {"zimmernummer": "Zimmernummer", "zimmertyp": "Zimmertyp", "preis_pro_nacht": "Preis pro Nacht (€)", "max_personen": "Maximale Personenzahl", "foto": "Foto"}};
    ...
    {createZimmerOpen && (
      <ZimmerDialog
        ...
        defaultValues={createZimmerInitial
          ? ({ zimmernummer: createZimmerInitial } as any)
          : undefined}
      />
    )}
    </>
